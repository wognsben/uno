...
export default function MyPage() {
  return (
    <MyPageLayout
      ...
      description="우노트래블 예약, 장바구니, 문의, 회원정보를 관리하는 마이페이지 허브입니다."
      activePath="/mypage"
    >
      <div className="grid three">
        {[
          ["장바구니", "담아둔 투어와 예약 전환 전 상품을 확인합니다.", "/mypage/cart"],
          ["예약목록", "예약일, 투어일, 상품명, 예약 상태를 확인합니다.", "/mypage/reservations"],
          ["1:1 문의하기", "예약 변경, 취소, 현지 문의를 남깁니다.", "/mypage/inquiry"],
          ["개인정보 수정", "이름, 이메일, 연락처, 카카오톡 ID를 관리합니다.", "/mypage/profile"],
          ["투어 신청 / 예약 전환", "장바구니 상품을 실제 투어 신청으로 전환합니다.", "/mypage/tour"],
          ["로그아웃", "현재 임시 로그인 세션을 종료합니다.", "logout"],
        ].map(([title, desc, path]) => (
          <article className="card" key={title}>
            <div className="card-kicker">UNOTRAVEL</div>
            <h3>{title}</h3>
            <p>{desc}</p>
            <button className="card-link" type="button" onClick={() => path === "logout" ? logout() : navigateTo(path)}>
              {path === "logout" ? "로그아웃" : "이동하기"}&nbsp;→
            </button>
          </article>
        ))}
      </div>
